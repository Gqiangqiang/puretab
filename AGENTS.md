# AGENTS.md — TabSync 扩展

面向 AI 编码代理的项目指南。请先阅读本文件再开始改动。

## 项目概览

TabSync（"我们的标签页"）是一款 Manifest V3 浏览器扩展，通过 `chrome_url_overrides.newtab` 接管新标签页，提供最简洁的新标签页体验：时钟、搜索、快捷链接、锁屏等模块。
...
- 打包发布：`./pack.sh`。脚本从 `manifest.json` 读取 `version`，生成 `dist/tabsync-v<version>.zip`（仅含 manifest/html/css/js/icons）。
...
### 单类驱动
所有逻辑集中在 `js/app.js` 的 `TabSyncApp` 类中，按模块拆分为 `initXxx()` 方法。新增功能时：
- 在 `init()` 中调用新的 `initXxx()`
- 用 `// ====` 包裹的中文注释块划分模块边界（保持现有风格）
- 实例在 `window.tabSync` 上，供 HTML 内联事件或 DevTools 调试使用
...
- ⚠️ `initAnniversary()`（纪念日提示）当前在 `init()` 中被注释禁用，代码仍保留。

### 数据持久化
- 使用 `chrome.storage.local`（通过 `StorageWrapper` 类包装），键名统一前缀 `tabsync_`（如 `tabsync_links`、`tabsync_theme`）
...
- 首次运行自动迁移旧 `localStorage` 数据（检测 `tabsync_` 前缀键并复制到 `chrome.storage.local`）
- 开发降级：非扩展上下文下（如直接在浏览器打开 newtab.html）自动回退到 `localStorage`
- ⚠️ `loadData()` 当前是占位逻辑（仅读 `tabsync_theme` 且未应用回 UI）。真正的持久化加载分散在 `loadPreferences()` + 各模块自己的 `initXxx()`（各自读取所需 key）。**新增持久化字段：在对应 `initXxx()` 内读、在对应写操作处存即可，不要依赖 `loadData()`。**

### 主题系统
- `css/style.css` 顶部 `:root` 定义「晨光」浅色主题 CSS 变量，`[data-theme="dark"]` 覆盖为深色
- 切换通过 `document.documentElement.setAttribute('data-theme', ...)`，状态存 `tabsync_theme`
...
- 反馈：`showToast(msg)` 提示、`triggerConfetti(el)` 庆祝特效
- 搜索：自动识别 URL（正则匹配）直接跳转，否则 Google 搜索
...
- 设置项：`tabsync_auto_lock_enabled`（默认 `false`）、`tabsync_auto_lock_time`（秒，默认 60）
